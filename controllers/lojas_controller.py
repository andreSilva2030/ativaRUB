...
def edit(id):
    ...
    if request.method == 'POST':
        ...
        loja.qtd_sku = request.form.get('qtd_sku', type=int)
    ...
    return render_template('lojas/edit.html', loja=loja, divisoes=divisoes, grupos=grupos)
...
def api_update(id):
    ...
    if 'endereco' in data:
        loja.endereco = data['endereco']
    if 'qtd_sku' in data:
        loja.qtd_sku = data['qtd_sku']
    if 'id_divisao_bandeira' in data:
        loja.id_divisao_bandeira = data['id_divisao_bandeira']
    if 'id_grupo_trabalho' in data:
        loja.id_grupo_trabalho = data['id_grupo_trabalho']
    try:
        db.session.commit()
        return jsonify(loja.to_dict())
    except Exception as e:
